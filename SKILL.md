...
1. **Resolve the source title and message first.** If the user/source data provides an
   explicit main title, that exact title becomes the chart's main title. Do not replace it
   with an invented editorial hook, metaphor title, conclusion, or poetic phrase. The
   message/conclusion may become a subtitle, deck note, or supporting annotation. Only
   when no source title is provided should you infer one concise audience conclusion as
   the main title and confirm briefly. One chart = one point. If the message is a single
   number, attach a comparison (see principle 1 below) - a lone number is never enough.
   In custom renderers, record `source_title` and `rendered_title`; when `source_title`
   exists, use `layout_guard.require_source_title(...)` or an equivalent assertion before
   saving.
...
2. **Design the whole information architecture before drawing.** Decide the page's
   ...
   **common-subject abstraction audit** before choosing an illustration subject. Ask what
   ...
   Same-level peer metrics must also use the same **visual treatment grammar**. A peer
   time point, category, row, or metric may not be rendered as a large badge/stamp/card,
   separate illustration module, independent mini-chart, oversized circle, or different
   shape family while the other peers remain plain labels, rows, or small marks. A last,
   minimum, maximum, current, or selected peer may receive subtle color, weight, underline,
   or stroke emphasis within the same geometry, but it must not switch to a different
   structural treatment unless it is explicitly reclassified as a focal/summary metric in
   the construction plan and no longer presented as the same peer group. For time-series
   data with the same unit and scope, all time points are same-level peers by default.
   In custom renderers, declare peer treatment IDs and use
   `layout_guard.require_peer_treatment_group(...)` or an equivalent assertion.
...
4. **Metaphor Object** (`style-3`) - inherited PPT background or cream fallback; a real
   object IS the chart, with source title preserved when supplied. For "viral / single
   punchy stat".
...
- Register same-level peer treatment, not only geometry. Geometry checks can miss a
  loophole where four peers use one label/card grammar and the fifth peer becomes a
  detached badge, seal, giant circle, separate illustration, or mini-chart outside the
  registered peer group. Before rendering, list every same-level source item and its
  structural treatment ID. The list must contain one treatment ID unless a metric has
  been explicitly reclassified as focal/summary/context and the visible hierarchy makes
  that reclassification clear. In custom renderers, call
  `layout_guard.require_peer_treatment_group(...)`. Color emphasis is not a different
  treatment; changing geometry, container type, mark family, or module scale is.
...
- Visible copy is for the audience, not for explaining the designer's method. Titles,
  ...
  image was made rather than what the audience should learn. If no source title exists, a
  metaphor title may be poetic, but the supporting copy should still read as a factual
  data statement. In custom renderers, pass visible strings into
  `layout_guard.add_text_box(..., text=...)` or `layout_guard.add_text(...)` so the copy
  guard can catch design-rationale wording.
...
- For generated metaphor objects, animal bodies, portraits, product heroes, and other
  ...
  **visible edge/boundary** of the object part, not the center of the object, body,
  ...
  small measured gap; do not drive the stroke into fur, skin, faces, product surfaces,
  dense texture, or the visual center merely because that point is easy to compute. For a
  peer group of leaders pointing to the same generated illustration, the target-side gap
  from each line endpoint to its corresponding visible edge must be equal after final
  crop/scale. As a default for 16:9 PNGs, use a 10-14 px gap with no more than 1.5 px
  spread across peer leaders. A line that enters the illustration is too close; a line
  that floats far away from the edge is too far, even if it points in the right direction.
  In custom renderers, use `layout_guard.add_edge_target_connector_path(...)` for these
  cases, pass a protected interior keepout zone, and call
  `layout_guard.require_connector_edge_gap_group(...)` or an equivalent assertion with
  inspected true edge points. Target-side endpoint dots are disabled by default for
  illustration-as-chart assets; if a marker is absolutely necessary, place a small tick or
  dot in adjacent empty space outside the subject edge and document its semantic role. Do
  not place a circular endpoint marker on the illustration body.
...
- The source/user data provides a main title, but the rendered chart uses an invented
  headline, metaphor hook, conclusion sentence, or date-range label as the main title.
  Source title preservation is mandatory; hooks belong in subtitles or notes.
...
- Same-level peer data uses mixed structural treatment without explicit reclassification:
  for example one time point becomes a large badge, stamp, circle, separate card, or
  independent mini-chart while the other time points remain labels or small marks.
...
- A peer group of leaders pointing to the same generated illustration has unequal
...
- If no source title exists and a witty/conceptual title is generated, it must be
  **original wordplay** - never reproduce song lyrics, film quotes, or other copyrighted
  lines. If a source title exists, preserve it as the main title.
- Do not reproduce a specific published infographic; rebuild palettes/layouts as
  original templates.

## QC checklist (before presenting)

- [ ] Source-title fidelity verified. If the user/source data supplied a main title, the
      rendered main title matches it exactly; any inferred conclusion, metaphor hook, or
      date range is placed as subtitle/supporting copy instead.
- [ ] If no source title exists, the title states a conclusion, not just a topic; one
      chart = one point.
...
- [ ] Same-level peer treatment passed a coded check. Every same-level source item was
      assigned a structural treatment ID, and peers share one treatment grammar. A
      selected/min/max/current peer may change color or weight, but it does not become a
      separate badge, stamp, giant circle, independent card, mini-chart, or different
      shape family unless explicitly reclassified as focal/summary/context.
...
- [ ] Connector edge-gap equality verified for generated illustrations. For peer leaders
      pointing to the same object/animal/product/image, each graphic-side endpoint stops
      the same measured distance from the corresponding visible edge/contour after final
      crop/scale, using `layout_guard.require_connector_edge_gap_group(...)` or an
      equivalent assertion. No line enters the illustration body, and no line floats far
      from the edge.
