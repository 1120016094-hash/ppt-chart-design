...
2. If the source data supplies a main title, use that exact title as the poster title.
   If no source title exists, give it an **original** pun/hook title (no copyrighted
   lyrics/quotes). A hook may become a subtitle or small editorial line, but it must not
   replace a provided source title.
...
- Same-level data points must keep the same structural label/mark grammar. A last,
  smallest, largest, current, or selected point may use stronger color or weight within
  the same label rail, but it may not become a detached stamp, circular badge, separate
  card, different object, or oversized module while peer points stay as plain labels. If
  that point is meant to become a focal summary, reclassify it before layout and remove it
  from the peer group rather than silently changing its treatment.
...
- For a peer group of leader lines that point to one generated object/animal/product, all
  target-side endpoints must stop the same measured distance outside the corresponding
