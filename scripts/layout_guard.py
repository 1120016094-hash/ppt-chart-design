...
class LayoutGuard:
    ...
    def require_source_title(self, name: str, source_title: str, rendered_title: str) -> None:
        """Require the main chart title to preserve the source title exactly.

        Use this when the user/source data provides an explicit main title. Creative
        hooks, conclusions, date ranges, and editorial summaries should become subtitles
        or supporting copy, not replacements for the source title.
        """
        self.source_title_requirements.append((name, source_title, rendered_title))

    def require_peer_treatment_group(self, name: str, treatment_ids: Sequence[str]) -> None:
        """Require same-level peer data to use one visual grammar.

        ``treatment_ids`` should describe structural treatment, not color emphasis:
        for example ``label_rail_leader``, ``same_card``, ``same_bar_row``. A selected
        item may use a different fill/color, but it should not switch to a badge, stamp,
        independent card, separate chart, or larger geometry unless it has been
        reclassified as a non-peer focal/summary metric before rendering.
        """
        treatments = [t.strip().lower() for t in treatment_ids]
        if len(treatments) < 2:
            raise ValueError("peer treatment group requires at least two treatments")
        self.peer_treatment_requirements.append((name, treatments))

    def require_connector_edge_gap_group(
        self,
        name: str,
        endpoint_edge_pairs: Sequence[Tuple[Point, Point]],
        expected_gap: float = 12,
        tolerance: float = 1.5,
        equal_tolerance: float = 1.5,
    ) -> None:
        """Require leader endpoints to stop an equal distance from true image edges.

        Each pair is ``(connector_endpoint, true_visible_edge_point)`` in final canvas
        coordinates. Use this for generated-object/animal/product illustrations where the
        leader should point to the silhouette/contour/segment edge without entering the
        object or floating too far away. The declared edge point must be the inspected
        visible boundary, not an arbitrary hand-authored target zone.
        """
        pairs = [
            ((float(endpoint[0]), float(endpoint[1])), (float(edge[0]), float(edge[1])))
            for endpoint, edge in endpoint_edge_pairs
        ]
        if len(pairs) < 2:
    ...
    def assert_clear(self, extra_forbidden: Sequence[Box] = ()) -> None:
        ...
        for name, source_title, rendered_title in self.source_title_requirements:
            if source_title.strip() and rendered_title.strip() != source_title.strip():
                failures.append(
                    f"{name} rendered main title {rendered_title!r} does not match source title {source_title!r}"
                )
        for name, treatments in self.peer_treatment_requirements:
            unique = sorted(set(treatments))
            if len(unique) > 1:
                failures.append(
                    f"{name} same-level peers use mixed visual treatments: {', '.join(unique)}"
                )
        for name, pairs, expected_gap, tolerance, equal_tolerance in self.connector_edge_gap_groups:
            distances = [math.hypot(endpoint[0] - edge[0], endpoint[1] - edge[1]) for endpoint, edge in pairs]
            for i, distance in enumerate(distances):
                if abs(distance - expected_gap) > tolerance:
                    failures.append(
                        f"{name} connector {i} edge gap is {distance:.1f}px, "
                        f"expected {expected_gap:.1f}px ± {tolerance:.1f}px"
                    )
            if max(distances) - min(distances) > equal_tolerance:
                failures.append(
